...
| 4 Variant B (Cactus on Mac) | same | one canonical worked example (the invert shader) baked into the system prompt; no rules | **32 / 50 = 64% — slight regression** vs baseline 68% but +5 absolute over Variant A | Few-shot binds reliably where rules didn't. CRT jumped 2/5 → **5/5** (model imitated the example structure). Underwater +1 to 5/5. But sepia dropped 5/5 → 3/5 and glitch 4/5 → 2/5 — the model imitated the *simple* invert pattern for concepts that needed elaborate code. Implication: a single example **biases toward its shape**. Round-4 next step: two diverse examples. |
| 4 Variant C (Cactus on Mac) | same | two diverse worked examples (invert + 3×3 box blur) | **34 / 50 = 68% — exact tie with baseline** | Three rounds of broad-prompt search, same total score, just shuffled per-concept noise. Confirmed: full-batch sweeps are too coarse and too noisy to drive iteration. Pivoted to per-concept hill climbing. |
| Hill: neon-1 (5 variants × 5 runs) | per-concept | inline rule, snippet, full-shader few-shot, rephrased prompt | baseline 5/5 — every intervention regressed | Sampling variance at 5 runs is huge; neon already at ceiling on Mac. Wrong target. Bumped to 10 runs and picked a concept with real headroom. |
| Hill: pixelate-1 (5 variants × 10 runs) | per-concept | inline rule, **snippet**, full shader, rephrased prompt | baseline 3/10 → **P2 snippet 10/10**; P3 full shader 10/10; P4 rephrase 10/10; P1 rule 2/10 (regressed) | Decisive. 4-line snippet hits 100% from 30%. Model literally regurgitates the snippet line-for-line. Locked. |
| Hill: thermal-1 (5 variants × 10 runs) | per-concept | inline rule, **snippet**, full shader, rephrased prompt | baseline 1/10 → **T2 snippet 10/10**; T3 full shader 10/10; T1 rule 3/10; T4 rephrase 5/10 | Same pattern — 7-line snippet hits 100% from 10%. Locked. |
| 5 Stacked validation (Cactus on Mac) | same | baseline + pixelate-P2 snippet + thermal-T2 snippet | **37 / 50 = 74% — +6 absolute over baseline** | Both targeted concepts hit 5/5 as predicted (pixelate +3, thermal +3). CRT and vignette also +2 each (likely benefit from the more concrete examples in the prompt). Neon and underwater each -2 — ambiguous whether real regression or noise; investigating in next round. **Locked into App.tsx's `SHADER_SYSTEM_PROMPT`.** |
