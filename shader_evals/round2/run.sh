#!/usr/bin/env bash
# Round 2: same 5 prompts as round 1 against gemma4:e2b via ollama.
# System prompt picks up the production prompt from App.tsx (SHADER_SYSTEM_PROMPT)
# — adds an explicit "clamp final RGB to [0,1]" directive aimed at fixing the
# underwater-blowout failure observed in round 1.
set -euo pipefail
